...
public partial class GameManager : Node
{
	...
	public override void _Ready()
	{
		...
		_gameModeButton = GetNode<OptionButton>("GameUI/SharedControlPanel/GameModeButton");
		
		//Populate game mode options
	...
	private void OnStartButtonPressed()
	{
		...
		_isVsBot = (_gameModeButton.GetSelectedId() == 1);
	...
	{   
		//AI evalutes its hand and plays a card if it can reach the target window
	...
	private void TryAiPlayModifierCard()
	{
		...
		foreach(Card card in _player2.ModifierHand)
		{
			...
			//Scenario A: AI has busted and needs a negative card to save itself safely under target
			...
			//Scenario B: AI is under target and can play a card to get closer to target without busting
		...
		{
			GD.Print($"AI Bot players modifier {bestCardToPlay.CardName}. New Score: {projectedScore} (Target: {target})");
			Control boardContainer = GetNode<Control>("GameUI/Player2Side/BoardSlotsContainer");
			_player2.PlayModifierCard(bestCardToPlay, _gameState);
			InstantiateCardView(bestCardToPlay, boardContainer);
